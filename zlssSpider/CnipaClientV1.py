...
from pymongo import errors,MongoClient,WriteConcern, ASCENDING
...
class Patentspider:
    ...
    # }
        ...
        self.zlxx_item=[]
        self.local_conn=Redis("192.168.6.167",14228,0,"uf$vU_1~wA0mB@Z+")
        self.db=MongoClient("localhost",27017)

        self.zlxx_filter_key = self.db[area]["filter_zlxx_com"]
        self.zlxx_filter_key.create_index([('company', ASCENDING)], unique=True)

        self.zlxx_comp_key =area+":zlxx_id"
                    ...
                    if comp_name in processed_companies:
                        ...
                        if token1 and content1 and ts_code1 and js_path1:
                            for _type in [
                                "1",
                                "3","4","6","9","10"]:
                                flist.append(executor.submit(self.details_response,totalpage,token1, content1, ts_code1, js_path1,comp_name,data,lastAn,lastGgr,_type))
                        _ += 1
                        if len(flist) >= 5:
